...
private boolean isSpinUp = false;
  ...
  // 
  public Shooter() {
    configMotor(shootMotor, false, 80.0); // Configures the motor with counterclockwise rotation positive and 80A current limit. 
  }

  //Turns on motor
  public void spinUp() {
    shootMotor.setControl(new VelocityVoltage(80.0).withEnableFOC(true)); // Sets the velocity of the motor in rotations per second.
    isSpinUp = true;
  }

  //Turn off motor
  public void spinDown() {
    shootMotor.setControl(new VelocityVoltage(0.0).withEnableFOC(true)); // Sets the velocity of the motor in rotations per second.
      isSpinUp = false;
  }
