...
public void autonomousPeriodic() { 
    ...
    if (scanning) {
        ...
        if (sensorValue > bestDistance) {
            bestDistance = sensorValue;
            Angle = currentAngle;
        }
        if (currentAngle >= 360) {
            scanning = false;
    ...
    else if (Math.abs(bestAngle - currentAngle) > 2) {
        double error = Angle - currentAngle;
        m_drivetrain.arcadeDrive(0, error > 0 ? 0.4 : -0.4);
    }
    else {
        if (sensorValue > 0.05) {
