...
public class Robot extends TimedRobot {
  ...
  private final CANdle frontCandle = new CANdle(0, "canivore"); // Initializes the lights on the front of the robot.
  private final CANdle backCandle = new CANdle(1, "canivore"); // Initializes the lights on the back of the robot.
  ...
  private SolidColor SolidColorRequest = new SolidColor(0, 39); // Used to control the CANdle lights.
  ...
  private static final String auto7 = "really cool algae code that totally works";
  ...
  public void robotPeriodic() {
    ...
    frontCandle.setControl(SolidColorRequest.withColor(currentColor));
    backCandle.setControl(SolidColorRequest);
  }
  ...
  public void autonomousInit() {
    ...
    switch (autoSelected) {
      ...
      case auto7:
        ...
        swerve.pushCalibration(true, 90.0); // Updates the robot's position on the field.
        swerve.resetDriveController(scoringHeadings[21]); // Prepares the robot to drive to an algae.
      break;
      ...
      case auto7:
        switch (autoStage) { 
          ...
          // Auto 7, Stage 1 code goes here.
             swerve.driveTo(scoringPositionsX[21],scoringPositionsY[21],scoringHeadings[21]); // This moves the robot to an algae. x=5.832475, y=4.00685
            if (swerve.atDriveGoal()) {
              autoStage = 2; // Advances to the next stage once the robot is at the correct location.
            }
          break;
          ...
          // Auto 7, Stage 2 code goes here.
             swerve.drive(0.1, 0.0, 0.0, true, 0.0, 0.0); // Goes backwards very slowly.
            if (swerve.getXPos() >= 6.2) {
             swerve.drive(0.0, 0.0, 0.0, true, 0.0, 0.0); // Stops the robot's movement.
             autoStage = 3; // Advances to the next stage when the robot has moved backwards enough.
            }
          break;
          case 3:
          // Auto 7, Stage 3 code goes here.
             swerve.drive(0.0, 0.0, 0.0, true, 0.0, 0.0); // Holds the robot still.
             elevator.setLevel(Level.highAlgae); // This moves the elevator to the high algae position.
            if (elevator.atSetpoint()) {
             algaeYeeter.setArmPosition(ArmPosition.algae); // Makes it ready to grab algae.
             autoStage = 4; // Advances to the next stage once the elevator is at the correct level and is ready to grab algae.
            }
          break;
          case 4:
          // Auto 7, Stage 4 code goes here.
             swerve.drive(-0.1, 0.0, 0.0, true, 0.0, 0.0); // Goes forward very slowly.
            if (swerve.getXPos() <= 5.8) {
             swerve.drive(0.0, 0.0, 0.0, false, 0.0, 0.0); // Stops the robot's movement.
             autoStage = 5; // Advances to the next stage when the robot has moved forwards enough.
            }
          break;
          case 5:
          // Auto 7, Stage 5 code goes here.
            ...
            if (algaeYeeter.algaeDetected()) {
             autoStage = 6; // Advances to the next stage once algae is detected in the yeeter.
            }
          break;
          case 6:
          // Auto 7, Stage 6 code goes here.
             swerve.drive(0.0, -0.1, 0.0, false, 0.0, 0.0); // Goes backwards very slowly.
            if (swerve.getXPos() >= 6.2) {
             swerve.drive(0.0, 0.0, 0.0, false, 0.0, 0.0); // Stops the robot's movement.
             autoStage = 7; // Advances to the next stage when the robot has moved backwards enough.
            }
          break;
          case 7:
          // Auto 7, Stage 7 code goes here.
             ...
             elevator.setLevel(Level.bottom); // This moves the elevator to the bottom level.
            if (elevator.atSetpoint()) {
            swerve.resetDriveController(scoringHeadings[29]); // Prepares the robot to move to the net.
             //autoStage = 8; // Advances to the next stage once the elevator is at the correct level.
            }
          break;
          case 8:
          // Auto 7, Stage 8 code goes here.
             swerve.driveTo(scoringPositionsX[29], scoringPositionsY[29], scoringHeadings[29]); // Brings the robot to the net.
            if (swerve.atDriveGoal()) {
          ...
          // Auto 7, Stage 9 code goes here.
             swerve.drive(0.0, 0.0, 0.0, false, 0.0, 0.0); // Holds the robot still.
             elevator.setLevel(Level.L4); // This moves the elevator to the fourth level.
            if (elevator.atSetpoint()) {
              algaeYeeter.yeet(); // Yeets the algae.
              autoStage = 10; // Advances to the next stage once the elevator is at the correct level.
            }
          break;
          case 10:
          // Auto 7, Stage 10 code goes here.
             swerve.drive(0.0, 0.0, 0.0, false, 0.0, 0.0); // Holds the robot still.
            if (!algaeYeeter.isYeeting()) {
             autoStage = 11; // Advances to the next stage once the algae is yeeted.
            }
          case 11:
  ...
  public void teleopPeriodic() {
    ...
    if (swerveLock) {
      ...
      if (swerve.getXPos() >= 4.5 && swerve.getXPos() <= 7 &&
          swerve.getYPos() >= 6   && swerve.getYPos() <= 7) {
      ...
      else if (swerve.getXPos() >= 3   && swerve.getXPos() <= 4 &&
               swerve.getYPos() >= 5.5 && swerve.getYPos() <= 6.5) {
            SmartDashboard.putString("Auto Drive", "In Zone: Press A-Left/B-Right/X-Algae");
          if (driver.getRawButton(1)) {// A
